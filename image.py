...
class ImageDataGenerator(object):
    ...
    def get_random_transform(self, img_shape, seed=None):
        ...
        transform_parameters = {'theta': theta,
                                ...
                                'histogram_equalization' : self.equalization,
                                'gamma' : self.gamma,
    ...
    def apply_transform(self, x, transform_parameters):
        ...
        if transform_parameters.get('contrast_stretching') is True:
            p2, p98 = np.percentile(x, (2, 98))
            x = exposure.rescale_intensity(x, in_range=(p2, p98))

        if transform_parameters.get('logarithmic') is True:
            x = exposure.adjust_log(x, 1)

        if transform_parameters.get('gamma') is True:
            x = exposure.adjust_gamma(x, 2)

        if transform_parameters.get('histogram_equalization') is True:
            x = exposure.equalize_hist(x)

        if transform_parameters.get('adaptive_equalization') is True:
